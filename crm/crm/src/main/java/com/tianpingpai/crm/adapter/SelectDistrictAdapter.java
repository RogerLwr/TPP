...
public class SelectDistrictAdapter extends BaseAdapter {
        ...
        if(isOnly){
            ...
            if(disData.size()!=0){
                for(int i=0;i< disData.size();i++){
                    if(selectedId.contains(disData.get(i).get("id"))){
                        toggleSelection(i, disData.get(i).get("name")+"abc"+ disData.get(i).get("id"));
                    }
                }
            }
        }else{
    ...
    public View getView(final int i, View view, ViewGroup viewGroup) {
        ...
        view.findViewById(R.id.selection_image_view).setOnClickListener(new View.OnClickListener() {
            ...
            toggleSelection(i,disData.get(i).getString("name"));
            int p = Integer.getInteger((String) disData.get(i).get("id"));

            toggleSelection(i,(String)disData.get(i).get("name")+"abc"+disData.get(i).get("id"));
            Log.e("xx", "onClick");
            }
        });
        */
            ...
            if(0 == i){
                if(selectionItem.containsKey(i)){
                    ...
                    for(int a = 0;a<disData.size();a++){
                        selectionItem.put(a,disData.get(a).get("name") + "abc" + disData.get(a).get("id"));
                        if(a>0){
                            selection.put(a,disData.get(a).get("name") + "abc" + disData.get(a).get("id"));
                        }
                    }
                }
            }else{
