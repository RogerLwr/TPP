...
public class BuyerCustomersViewController extends CrmBaseViewController {
    ...
    private BuyerCustomerFilterViewController filterViewController;
...
//            if(filterAdapter.getSelectedCustomerSource() != 0){
//                req.addParam("source",filterAdapter.getSelectedCustomerSource()+"");
//            }
//            if(filterAdapter.getSelectedCustomerStatus() != 0){
//                req.addParam("order_tag",filterAdapter.getSelectedCustomerStatus()-1+"");
//            }
        }else {
...
//            req.addParam("user_type", userType);//默认是买家1 个人中心进入的我的客户是全部  -1
    ...
    private HttpRequest.ResultListener<ListResult<Model>> customersListener = new HttpRequest.ResultListener<ListResult<Model>>() {
        @Override
    ...
    private void loadCustomers(int page) {
        ...
        if (filterViewController != null) {
            ...
            if(filterAdapter.getSelectedNotOrderStatus() != 0){
                req.addParam("last_order_time",filterAdapter.getSelectedNotOrderStatus() + "");
            }
    ...
    private void showFilter() {
        ...
        /*
        if(filterViewController == null){
            filterViewController = new BuyerCustomerFilterViewController();
            filterViewController.setActionSheet(getActionSheet(true));
            filterViewController.setOnConditionSelectedListener(new BuyerCustomerFilterViewController.OnConditionSelectedListener() {
                @Override
        ...
        */
        ...
        if(actionSheet == null){
            ...
            actionSheet.setWidth(DimensionUtil.dip2px(300));
            actionSheet.setHeight(getView().getHeight());
            actionSheet.setActivity(getActivity());
            filterViewController = new BuyerCustomerFilterViewController();
            filterViewController.setActionSheet(actionSheet);
            filterViewController.setOnConditionSelectedListener(new BuyerCustomerFilterViewController.OnConditionSelectedListener() {
                @Override
                public void onConditionSelected() {
                    loadCustomers(1);
            ...
            actionSheet.setViewController(filterViewController);
        }
        actionSheet.show();
    }
}
