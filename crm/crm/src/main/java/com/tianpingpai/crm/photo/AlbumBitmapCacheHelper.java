...
//        cr = AppContext.getInstance().getContentResolver();
     ...
     * @param objects
     */
    public void getBitmapFromPath(final String path, int width, int height, final ILoadImageCallback callback, Object... objects){
        decodeBitmapFromPath(path, width, height, callback, objects);
    }
...
//                }
                ...
                if (width == 0 || height == 0) {
                    ...
                    options.inSampleSize = computeScale(options, ((WindowManager) (CrmApplication.getInstance().getSystemService(Context.WINDOW_SERVICE))).getDefaultDisplay().getWidth(), ((WindowManager) (CrmApplication.getInstance().getSystemService(Context.WINDOW_SERVICE))).getDefaultDisplay().getWidth());
                    ...
                    Log.e("kkk","//无法在临时文件的缩略图目录找到该图片，于是执行第二步");
                    if (bitmap == null) {
                        BitmapFactory.Options options = new BitmapFactory.Options();
                        options.inJustDecodeBounds = true;
                        BitmapFactory.decodeFile(path, options);
...
//                        }
                        //yao shanchu
                        if(bitmap == null){
                            Log.e("bitmap","weikongle======");
                        }
                        //第三步,如果缩放比例大于4，该图的加载会非常慢，所以将该图保存到临时目录下以便下次的快速加载
                        Log.e("kkkk","//第三步,如果缩放比例大于4，该图的加载会非常慢，所以将该图保存到临时目录下以便下次的快速加载");
                        if (options.inSampleSize>=4 && bitmap!=null) {
                            try {

                                Log.e("12345",tempPath);
                ...
                if (bitmap != null && cache!=null){
                    Log.e("kkkkk","bitmap you shu ju");
                    cache.put(path +"_"+ width +"_"+height, bitmap);
                }else{
                    Log.e("kkkkk","bitmap mei shu ju");
                }

                Log.e("sdsdsdwewe","tupian"+path);
